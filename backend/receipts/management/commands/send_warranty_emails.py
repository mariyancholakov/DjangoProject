...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        current_time = now()
        start_window = current_time + timedelta(days=28)
        end_window = current_time + timedelta(days=30)
        
        self.stdout.write(f"Checking warranties expiring in around 30 days")

        receipts = Receipt.objects.filter(warranty_months__isnull=False)
        ...
        for receipt in receipts:
            ...
            if expiry_date:
                if start_window <= expiry_date <= end_window:
                    user = receipt.user
                    if user and user.email:
                        send_mail(
                            ...
                            message=(
                                ...
                                f'has a warranty that will expire on {expiry_date.date()}.\n\n'
